...
import { ThemeOption, CategoryStyle } from '../../types';
import { sendMorningTime, sendShutdownTime } from '../../utils/electronBridge';
...
interface SettingsModalProps {
    ...
    customCategories: Record<string, CategoryStyle>;
    setCustomCategories: (categories: Record<string, CategoryStyle>) => void;
    onOpenThemeCreator: () => void;
    shutdownTime: string;
    onSetShutdownTime: (time: string) => void;
    morningTime: string;
    onSetMorningTime: (time: string) => void;
    soundEffectsEnabled: boolean;
    onSetSoundEffectsEnabled: (enabled: boolean) => void;
    autoArchiveEnabled: boolean;
    onSetAutoArchiveEnabled: (enabled: boolean) => void;
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ 
    ...
    onOpenThemeCreator, shutdownTime, onSetShutdownTime, morningTime, onSetMorningTime,
    ...
    autoStartEnabled, onSetAutoStartEnabled
}) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-lg z-50 flex items-center justify-center p-4" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
            <motion.div ref={trapRef as any} initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-md bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-2xl p-6 overflow-y-auto max-h-[90vh]">
                ...
                <div className="mb-6">
                     ...
                     <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between bg-[var(--color-bg)] p-3 rounded-lg">
                            <span>Start Aura on Windows Startup</span>
                            <button onClick={() => onSetAutoStartEnabled(!autoStartEnabled)} className={`w-12 h-6 rounded-full p-1 transition-colors ${autoStartEnabled ? 'bg-[var(--color-accent)]' : 'bg-gray-500'}`}>
                                <motion.div layout className={`w-4 h-4 bg-white rounded-full ${autoStartEnabled ? 'ml-auto' : ''}`} />
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-6">
                    <h3 className="font-semibold text-[var(--color-text-primary)] mb-3">Productivity</h3>
                     <div className="flex flex-col gap-2">
                         <div className="flex items-center justify-between bg-[var(--color-bg)] p-3 rounded-lg">
                             <label htmlFor="morningTime">Start of Day Time</label>
                             <input 
                                 type="time"
                                 id="morningTime"
                                 value={morningTime}
                                 onChange={e => {
                                     onSetMorningTime(e.target.value);
                                     sendMorningTime(e.target.value);
                                 }}
                         ...
                         <div className="flex items-center justify-between bg-[var(--color-bg)] p-3 rounded-lg">
                            ...
                            <input 
                                ...
                                value={shutdownTime}
                                onChange={e => {
                                     onSetShutdownTime(e.target.value);
                                     sendShutdownTime(e.target.value);
                                }}
                                className="bg-transparent border-none text-[var(--color-text-primary)] focus:outline-none"
                            />
                        </div>
