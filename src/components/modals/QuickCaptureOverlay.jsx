...
/**
 * QuickCaptureOverlay — A minimal, always-on-top overlay window 
 * triggered via Ctrl+Shift+Space (Electron global shortcut).
 * In browser mode it functions as a floating quick-add widget.
 */
export const QuickCaptureOverlay = ({ onAddTask, onClose }) => {
    ...
    useEffect(() => {
        // Listen for tasks added from Electron main process
        if (window.electronAPI?.onAddTaskFromCapture) {
            const unsub = window.electronAPI.onAddTaskFromCapture((taskText) => {
                onAddTask(taskText);
            });
            return unsub;
        }
    ...
    const handleSubmit = () => {
        ...
        if (window.electronAPI?.submitQuickCapture) {
            // Electron: send to main process which forwards to main window
            window.electronAPI.submitQuickCapture(trimmed);
        } else {
            // Browser: add directly
            onAddTask(trimmed);
        }
        setText('');
        onClose?.();
    ...
    const handleKeyDown = (e) => {
        ...
        if (e.key === 'Escape') {
            if (window.electronAPI?.cancelQuickCapture) {
                window.electronAPI.cancelQuickCapture();
            }
            onClose?.();
        }
    };
