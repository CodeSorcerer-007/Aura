...
export const ArchiveModal: React.FC<ArchiveModalProps> = ({ isOpen, onClose, archivedTasks, onRestore, onDelete }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-lg z-[60] flex items-center justify-center p-4" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
            <motion.div ref={trapRef as any} initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-lg bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-2xl p-6">
                    ...
                    {archivedTasks.length > 0 ? archivedTasks.map(task => (
                        <div key={task.id} className="p-3 bg-[var(--color-bg)] rounded-lg flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-[var(--color-text-secondary)]">Completed: {task.completionDate ? formatDate(task.completionDate) : 'Unknown'}</p>
                            </div>
