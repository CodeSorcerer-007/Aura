...
import { Task, Distraction } from '../../types';

interface FocusViewProps {
    task: Task;
    onClose: () => void;
    onComplete: (taskId: string, distractions: Distraction[]) => void;
    onLogDistraction?: (text: string) => void;
}

export const FocusView: React.FC<FocusViewProps> = ({ task, onClose, onComplete, onLogDistraction }) => {
    const [duration, setDuration] = useState(25);
    const [timeLeft, setTimeLeft] = useState(duration * 60);
    const [isActive, setIsActive] = useState(false);
    const [soundType, setSoundType] = useState<string>('off');
    const [distractionText, setDistractionText] = useState('');
    const [showDistractionInput, setShowDistractionInput] = useState(false);
    const [distractionsThisSession, setDistractionsThisSession] = useState<Distraction[]>([]);
    const soundPlayer = useRef<any>(null);
    ...
    useEffect(() => {
        ...
        if (soundType !== 'off') {
            try {
                const player = new (Tone as any).Noise(soundType).toDestination();
    ...
    const handleLogDistraction = () => {
        ...
        if (!trimmed) return;
        const newDistraction: Distraction = { text: trimmed, time: new Date().toISOString() };
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/90 backdrop-blur-xl z-50 flex items-center justify-center p-4" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
