...
export const BottomNav: React.FC<BottomNavProps> = ({ currentView, setCurrentView }) => {
    ...
    return (
        <div className="flex items-center gap-1 sm:gap-2 bg-[var(--color-bg-secondary)]/80 backdrop-blur-lg border border-[var(--color-border)] rounded-full p-2 pointer-events-auto shadow-lg">
            {navItems.map(item => (
                <button
                    key={item.id}
