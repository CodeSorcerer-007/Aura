...
export const FilterBar: React.FC<FilterBarProps> = ({ activeFilter, setActiveFilter, categories, allTags }) => {
    const mainFilters: { type: ActiveFilter['type']; label: string }[] = [
        { type: 'all', label: 'All' },
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-6">
            {mainFilters.map(filter => (
                <button
                    ...
                    className={`px-3 py-1 text-sm rounded-full transition-colors ${activeFilter.type === filter.type && (activeFilter.value === null || activeFilter.value === undefined) ? 'bg-[var(--color-text-primary)]/90 text-[var(--color-bg)] font-semibold' : 'bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary-hover)]'}`}
                >
                    {filter.label}
                </button>
            ))}
